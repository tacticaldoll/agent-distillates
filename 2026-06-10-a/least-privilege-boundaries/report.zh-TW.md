# 最小權限的現代形狀：從降權到 Sandbox 邊界

**Structure**: Analytical Essay
**Date**: 2026-06-10T16:56
**Model**: GPT-5.5
...
「不要用 root 跑服務」是一句正確但不完整的建議。Linux 的最小權限（least privilege）不是單一開關，而是 process identity、groups、capabilities、namespace、seccomp、LSM 與 cgroup 共同形成的邊界。

本文的問題是：一個 app 或 daemon 要做到最小權限，實際依賴哪些層？答案是：讓 process 在每個生命週期階段只保留必要能力，並把可見世界、可用資源與可呼叫 syscall 一起縮小。
...
Daemon 常見的安全形狀不是永遠不用特權，而是只在初始化階段使用特權。初始化完成後，服務應進入低權限主迴圈。
...
最小權限的難點在於它不是單一維度。把 UID 降下去很重要，但如果 supplementary groups、capabilities、host socket mount 或 permissive syscall 面仍然存在，系統仍可能保留危險能力。
...
Linux 最小權限的通用原理是：不要只問 process 是不是 root，而要問它在每個維度上還保留什麼能力。

完整的最小權限模型至少包含四個問題：它以什麼 credentials 執行？它還帶有哪些 capabilities 與 groups？它在什麼 namespace 與 mount 視角裡？它能呼叫哪些 syscall、接觸哪些 host object、消耗多少資源？

當這些問題一起被回答，最小權限才從口號變成可檢查的工程狀態。
