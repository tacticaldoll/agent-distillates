# 自動化工作流的外部裁決：為什麼防禦不能只由 LLM 自己生成
...
本文的問題是：自動化工作流要如何避免變成「看起來有安全流程，實際沒有外部裁決」？核心答案是：自動化可以產生候選結構，但信任狀態必須由系統外部授權。
...
外部裁決不一定永遠是人類手動點頭。它也可以是測試、policy engine、permission boundary、type checker、runtime guard 或獨立 verifier。重點是它不能只是同一個 LLM 自己延續自己的敘事。
...
知識蒸餾、agent action、code review 與部署流程，需要的外部裁決不同。把它們全部交給同一種「AI 自評」是不夠的。

| 工作流 | AI 可自動化 | 必要外部裁決 |
|---|---|---|
| 知識蒸餾 | 摘要、結構、疑點清單 | 主題可信度、人類校驗、一手資料 |
...
這個流程看起來有效率，但所有判斷都在同一個自洽語境裡完成。它沒有真正的外部裁決。
...
這個流程不是反自動化，而是把自動化放在可控範圍內。AI 負責提高生成與檢查密度，外部裁決負責授權信任與行動。
...
可信的 AI 工作流需要外部裁決。這個裁決可以是人、測試、policy、permission boundary、獨立 verifier 或 context reset。共同原則是：
