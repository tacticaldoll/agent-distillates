# Permission Denied 不是一種錯：Linux 權限排查地圖
...
權限檢查的主體是 process，不是你的登入直覺。先確認程式實際以誰執行，以及它帶有哪些群組。
...
這張圖的重點是順序。先知道錯在哪類 object，再看主體身份與客體規則，最後檢查額外邊界。這比一開始就 `sudo` 更能保留根因。
