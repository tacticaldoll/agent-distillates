# 檔案、Socket 與 Kernel Object：權限檢查如何把主體接到客體
...
本文的核心論點是：filesystem 權限與 socket 權限不是兩套互斥世界。它們共享 process credentials 作為主體輸入，但客體的類型不同，所以檢查路徑不同。Unix socket file 正好站在兩者交界：它在 filesystem namespace 裡有名字與權限，但真正行為是 socket IPC。
...
Process credentials 解決的是主體身份問題：這個 process 現在以誰的身份執行、還保留哪些群組與能力。但權限判斷一定還有另一半：它要碰的東西是什麼。
...
從 kernel 視角看，process 是主體（subject），檔案或 socket 是客體（object）。權限結果不是任何一邊單獨決定，而是主體狀態、客體 metadata 與額外政策的合成。
...
Filesystem 與 socket 的共同點，是都把 process credentials 當成主體輸入。差異在於客體不同：regular file 的核心是 inode 與資料內容；Unix socket file 的核心是 pathname 入口與背後 socket endpoint。
