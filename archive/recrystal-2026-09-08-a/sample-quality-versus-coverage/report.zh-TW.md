# 品質分數為何看不見缺模式：生成評估的二維前緣

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T20:24
**Model**: Claude Opus 5
...
失效點是把「當前判別器騙過去了」當成「整體分佈覆蓋了」。前面那張表顯示，這兩件事在品質指標上完全無法區分。
...
在把低多樣性診斷為**模式坍縮**（Mode Collapse）之前，需要排除幾種會產生相似症狀的情況。
...
理論中的最優判別器正是散度推導所需要的東西——若判別器真的是 $D^*$，目標就等價於一個明確定義的分佈距離。問題出現在別的地方：有限支撐、飽和的損失形式、函數容量的限制，以及交替更新的動力，這些因素的**組合**才產生坍縮。
