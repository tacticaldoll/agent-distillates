# 「資訊還在狀態裡」為何不算證據：前向探測與反向梯度的分離

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T19:44
**Model**: Claude Opus 5
...
兩者共用同一個連乘，所以在這個最簡模型中它們同進同退。這正是「梯度消失」與「記憶消失」常被當成同一件事的原因。後面會看到，一旦引入截斷或門控學習，兩者就分開了。

先驗證這個共用結構的後果。下面的程式讓偏置 $b$ 成為可訓練參數，用梯度下降訓練兩萬步，看不同初始化的結局。
...
長序列訓練時，完整的反向傳播需要保存整段序列的中間狀態，記憶體開銷隨長度線性成長。標準的處理是**截斷反向傳播**：只在最近 $W$ 步之內回傳梯度，超過窗口的路徑直接切斷。
...
**當任務不需要長程資訊時，遺忘是正確行為。** 若損失函數從不獎勵保存第 1 步的內容，門控學到把它丟掉是最合理的解，不是故障。診斷長程失敗之前，必須先確認目標函數確實要求那個資訊。這個檢查常被跳過，因為「模型應該記住」是一個直覺而非設計。
...
實務後果是：看到梯度爆炸就加裁剪，然後發現長程準確率沒有改善，這不是矛盾。裁剪成功地做了它該做的事（防止非有限更新），而消失與**信用分配**（Credit Assignment）的問題原封不動。要區分兩者，需要同時記錄時間距離與梯度範數，而不只是看訓練是否穩定。
...
所以實驗證明的是**分離本身**——前向殘量與反向梯度可以脫鉤，而初始化只移動起點、不改變獎勵結構。它不能預測任何真實網路是否走得出去，因為走得出去取決於矩陣連乘的譜性質，那不在這個模型裡。
