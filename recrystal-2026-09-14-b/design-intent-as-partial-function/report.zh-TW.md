# 規格是一張查表：設計理念的全函數性質，與交接處丟失的那個定義域

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-14T11:45
**Model**: Claude Opus 5
...
再往下回推一層會更有意思。這條角色鏈的設計者通常讀過「概念完整性需要由少數幾個心智持有」這類主張，並認為自己遵守了它——畢竟寫規格的只有一個人。而實際上這條鏈把構思切成了兩半：「系統應該做什麼」由第一個角色決定，「要如何達成」由第二個角色決定，**兩者都是構思**，中間隔著一次交接。

本文要做的是把這件事從一個價值判斷改寫成一個可檢查的形式性質。設計理念是一個定義在整個情境空間上的**全函數**；規格是這個函數在一個有限子集上的**列舉**。交接傳遞的是列舉，而下游必須在列舉之外的地方求值。當它這麼做時，沒有任何東西會報錯。
...
這一點可以用型別直接展示。全函數在窮盡比對下，若情境空間擴張而未更新決定，編譯就會失敗；查表則在鍵不存在時回退到預設值，執行期正常結束。下面這段 Rust 程式把兩者並排，並額外計算跨模組相容約束在不同持有者數量下的歸屬情況。只用標準函式庫。
...
現在把情境空間擴張一個元素，也就是模擬「發現了一個新的邊界情況」，再用同一個編譯器檢查兩種表達的反應。查表版本照常編譯通過；全函數版本的窮盡比對則會失敗：
...
這個對比就是「構思可以被分工、不能被交接」的機械版本。**全函數強迫持有者在情境擴張時重新做決定；查表在同樣的情況下靜默地給出一個未經決定的答案。** 一份文件在結構上是查表，因此不論它寫得多完整，它在定義域外的行為都是回退而非報錯。
...
| 情境空間擴張，決策以全函數表達 | 新增 $c'$，窮盡比對未更新 | 編譯失敗 `E0004` | 強制持有者重新決定，不可繞過 |
...
兩個理由都是真的，而且都不是設計理由。它們解決的是組織問題，代價付在產物的一致性上，而那個代價不出現在任何人的績效表上。這一點與更廣泛的結構相連：人力彈性與責任歸屬有讀數，一致性沒有；於是有讀數的兩個被優化到最佳，沒讀數的那個被擠到邊界。

第三件值得記下的事是關於文件的上限。前面說交接傳遞結論而不傳遞產生結論的脈絡，這句話值得再壓一層。設計理念是一組**偏好**——在速度與一致性衝突時偏向哪邊、在通用與簡單衝突時偏向哪邊——而偏好只能從大量決定中被歸納，很難被直接陳述。這類難以言傳的知識如何在組織內轉移，其機制在知識管理的文獻中有系統的處理，見 [Nonaka，1994 / 《A Dynamic Theory of Organizational Knowledge Creation》](https://doi.org/10.1287/orsc.5.1.14)。
