# 規則書作為缺口地圖：惰性條文的析出、三種零效力，與混排下的單一折扣率
...
**因果機制**：規則的約束力來自違反的後果，而後果的前提是違反可被偵測。偵測缺席時，規則的語氣強度與它的實際效力完全脫鉤——一條用「必須」寫的條款與一條用「建議」寫的條款，在拒絕集合上都是空集。
...
**因果機制**：三者與被治理對象的關係不同——移除失去了檢查、死亡失去了對象、缺席從未擁有任何一個。關係不同因此可觀測性不同，可觀測性不同因此偵測手段不同。
...
下面這段程式把四種干預跑過一遍，並計算每種情況下讀者的後驗折扣率、互資訊與注意力誤配損失。只用 Node.js 標準函式庫。
...
第一段確認了機制本身。完全混排時似然比恰為 $1.000$，互資訊 $0.000000$ bits，後驗 $0.0500$ 等於先驗。完全分區時互資訊達到 $0.286397$ bits，**恰好等於先驗熵 $H(\pi)$**——也就是說，分區把關於效力的不確定性完全消除了。中間那一列則說明弱標記的效果有限：措辭略有不同使似然比升到 $1.286$，而互資訊只有 $0.008045$ bits，後驗從 $0.0500$ 只挪到 $0.0634$。

第二段是那個可證偽的判別。干預 A 與干預 B 的差別是條款總數從 100 降到 50，而混合比例維持 $0.05$；兩者的讀者折扣率都是 $0.0500$，**逐位元相同**。減量在混合比不變的前提下對折扣率沒有任何影響，容量論證在這個對照上被否證。

干預 C 則揭示了減量有時看起來有效的真正原因。只刪惰性條款、剩 52 條時，會咬人的佔比從 $0.05$ 升到 $0.0962$，折扣率同步升到 $0.0962$。**動的是混合比，不是數量**——而執行者通常把這次改善歸功於「規則變少了」。

干預 D 是對照的另一端。條款總數與混合比都沒有改變，只把會咬人的條款在版面上獨立呈現，折扣率從 $0.0500$ 跳到 $1.0000$。
...
| :--- | :--- | :--- | :--- |
| 100 條混排，$\pi=0.05$ | $\Lambda=1 \Rightarrow I(X;E)=0$ | 後驗 $=$ 先驗 | 折扣率 $0.0500$；誤配損失 $9.500$ |
| 等比例減量至 50 條 | $\pi$ 不變，$\Lambda$ 不變 | 無轉移 | 折扣率仍為 $0.0500$；容量論證被否證 |
| 只刪惰性條款，剩 52 條 | $\pi:0.05\to 0.0962$ | 先驗上移 | 折扣率 $0.0962$；有效，但原因是混合比 |
| 分區，條款數與 $\pi$ 皆不變 | $\Lambda\to\infty$，$I(X;E)=H(\pi)$ | 後驗分裂為 $1$ 與 $0$ | 折扣率 $1.0000$；誤配損失歸零 |
...
| 團隊反覆違反一條重要規則 | 混排使 $\Lambda=1$，讀者只能採單一折扣率 | 歸因為輕忽，加強宣導與懲處 | 把會擋下建置的條款在版面上獨立呈現 |
...
**邊界條件**：當讀者有外部資訊源時——例如它曾經親身撞過某條會咬人的規則——它可以對那一條維持單獨的後驗。這使得資深成員的折扣率結構優於新進成員，而這個差異本身是一個可觀察的診斷訊號。
...
第二個反思關於一個更大的同型問題。把一個放在流程末端的檢驗關卡視為品質保證，其實是對整條產線上所有未對帳接縫的單點替代。而當那個關卡的判定本身不是確定性的——當它依賴判斷、依賴抽樣、依賴一份可以有不同寫法的報告——它就具備了惰性條文的性質：它存在、它被執行、它產生一份通過的紀錄，而它是否真的攔住了什麼無法從紀錄中得知。**差別只在規模：一條惰性條文守著一道接縫，一個軟性的終端關卡守著全部接縫。**

第三件值得記下的事是同一個結構在其他產物上的重現。架構決策紀錄有完整記載的同型失效：沒有人標記舊紀錄為已取代，沒有人刪除，它們留在原處看起來權威，同時無聲地與程式碼庫矛盾。知識庫條目的情況相同。三者共用同一個性質——**它們都不可能出錯**。一份過期的決策紀錄不會讓任何建置變紅，不會與任何東西矛盾到被偵測，因此它不會被修正，只會累積。
...
錯誤的作法是把所有強制條款用統一格式編號條列，因為那看起來嚴謹、一致、專業。統一格式正是消除可區分性的手段——它讓似然比等於一，使讀者失去建立兩套折扣率的依據。
...
由此得到三個可遷移的判斷。第一，規則的約束力來自違反的可偵測性而非語氣強度：一條無法產生可觀測違反的強制條款，在效力上等同於一句期望，無論它用了多強的助動詞；撰寫規範時真正該決定的不是措辭，是這條規則被違反時什麼東西會知道。第二，零效力有缺席、移除與死亡三個來源，它們的可偵測性與正確處置各不相同——缺席的自訊息量為零，因此流程稽核在原理上偵測不到它，唯一可行的手段是對合法產物注入變異並清點存活者。第三，稀釋效應的機制是資訊而非容量：混排使似然比等於一、互資訊歸零，因此理性讀者只能採用先驗作為後驗；在混合比例不變的前提下減量對折扣率完全無效，而在數量不變的前提下分區可使誤配損失歸零。
