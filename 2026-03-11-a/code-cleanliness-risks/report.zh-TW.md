...
Agent 是接龍狀態機——它的「存在」等於 context window 中可見的 token。在這個前提下，傳統代碼品質實踐的多個面向都可能製造因果斷裂（causal breakpoint）：封裝刪掉了局部上下文、設計模式引入了 runtime 才確定的綁定、抽象層數超出了 agent 的追蹤能力。

本文展開這些風險在具體代碼實踐中的表現，從單點的語意稀釋到設計模式和語法層面的風險分級，再到多重實踐疊加後的系統性崩潰——包括二次污染和 spec-driven 開發的污染終態。

## 分析：單點風險向量

### 語意稀釋

語意稀釋（semantic dilution）是指一個精確的概念被泛化表述取代後，喪失了區分力。判斷標準是：**代碼結構暗示的自由度，是否在系統中被實際使用？**
...
高度抽象（high abstraction）本身不等於稀釋。判斷線是：抽象所暗示的自由度，是否有 **≥ 2 個消費者**在行使不同路徑？有，就是合理的抽象。沒有，就是語意稀釋。這個判斷標準也適用於過早泛化（premature generalization）——只有一個消費者卻建了 interface + factory + registry 的情況。
...
| **Decorator/Chain** | 高 | 行為在 runtime 疊加，靜態分析看不到最終組合，agent 對「這個呼叫實際做了什麼」的回答是概率性的 |
...
與設計模式類似，語法層面的風險也來自同一個根源：讓「看到的」和「執行的」不一致。以下覆蓋動態弱型別（JavaScript）、動態強型別（Python）、靜態強型別（Rust）三種語言，形成完整的風險光譜。
...
隱性異步上下文（implicit async context）是因果斷裂光譜中「從未存在」類型的極端案例。代碼語法上看起來是同步的，但實際執行涉及異步時序，而中間可能發生的狀態變更**從未以 token 形式出現在代碼文本中**。
...
以上所有風險向量——語意稀釋、高風險設計模式、危險語法、隱性異步——共享同一個特徵：**代碼的文本表述與實際執行行為之間存在落差**。人類用直覺、經驗和主動探索來補全這個落差。Agent 只有 token 序列，落差就是盲區。

## 反思：從單點到系統性崩潰

單一的因果斷裂造成局部錯誤。但當多個代碼實踐同時製造因果斷裂，風險從線性相加升級為指數放大。

### DRY + 集中知識庫的疊加放大

DRY 消除重複後，代碼路徑收斂為唯一一份。集中化知識庫（centralized knowledge base）收斂理解來源為唯一一處。兩者同時收斂，agent 的容錯餘裕（error margin）歸零：
...
本質上，DRY 收斂了代碼路徑，集中知識庫收斂了理解來源。兩者同時收斂等於 agent 的容錯餘裕歸零。人類在這個架構下靠記憶和直覺補全，agent 沒有這兩樣東西。

### 二次污染的靜默固化

疊加放大不是終點。當錯誤修改的結果被**反向寫回知識庫**，錯誤從暫時狀態固化為「事實」——這就是二次污染（secondary contamination）：
...
這條鏈是靜默的（silent）。每個階段看起來都像正常操作——代碼改了、文件同步了、下一個 session 正常工作。沒有任何環節會主動發出「這裡有污染」的信號。人類團隊中，code review 或某個「記得原本怎樣」的資深成員可能攔截。Agent session 之間無記憶延續，每次都是全新的信任起點——知識庫寫什麼就信什麼。

### Spec-driven 的污染終態

二次污染的最終形態是侵蝕規格（specification）本身。在雙層規格結構（dual-layer spec）中，Architecture 層的規則是「代碼贏——當代碼與 Architecture 矛盾時，更新 Architecture」。這條規則的設計意圖是允許代碼演進。但當代碼本身已被污染，這條規則就變成了**污染的高速公路**：
...
關鍵轉折點在於 Architecture → Requirement 的提升。這個提升的可逆性急劇下降：

| 階段 | 可逆性 |
|---|---|
| 代碼被污染 | 可逆——git revert |
| Architecture 被更新 | 可逆——但需要有人意識到它是錯的 |
| Architecture → Requirement 提升 | **幾乎不可逆**——後續所有 agent 都會「修正代碼以符合 spec」，將任何修復嘗試打回去 |

傳統開發中，過時的 spec 只是「沒人看的文件」。Spec-driven 開發中，spec 是強制執行的契約。過時的 spec 不會被忽略，它會被 agent **主動執行**。這把「文件過時」從被動風險轉變為主動破壞力。

## 結論

代碼品質實踐的風險可以分為三個層級。第一層是單點風險：語意稀釋、高風險設計模式、危險語法、隱性異步——每個都製造局部的因果斷裂。第二層是疊加風險：DRY + 集中知識庫同時收斂，agent 的容錯餘裕歸零。第三層是系統性崩潰：二次污染靜默固化，直到 spec-driven 框架開始主動保護錯誤。
