...
一個編碼器若對所有輸入都輸出標準常態，KL 成本可以降到零，解碼器卻無法從潛在變數區分樣本。這不是數值上的矛盾：變分自動編碼器（variational autoencoder, VAE）的目標同時獎勵資料擬合與近似後驗接近先驗，強解碼器有時可以繞過潛在通道完成前者。

Kingma 與 Welling 提出的自動編碼變分貝葉斯，目標是讓帶連續潛在變數、難解後驗的模型可用隨機梯度進行推論與學習；重參數化是其中的梯度估計工具。[原始論文](https://arxiv.org/abs/1312.6114) 本文的核心問題是：ELBO 經什麼機制建立可取樣的潛在空間，又在何時容許潛在資訊消失？
...
最後一項非負，因此證據下界（evidence lower bound, ELBO）確實是下界。第一項鼓勵 $z$ 支持重建，第二項使各輸入後驗不要任意偏離可取樣先驗。兩項作用於同一編碼通道，構成資訊與規則性的取捨。
...
$\mu=0,\sigma^2=1$ 時成本為零。從後驗坍縮（posterior collapse）的症狀回推：解碼器可在忽略 $z$ 時仍擬合條件分布；編碼器若攜帶輸入資訊便支付 KL；最佳化因此把 $q(z\mid x)$ 推向共同先驗；$I_q(X;Z)$ 下降，潛在變數停止區分輸入。失效點是把低 KL 當成良好正則化，而未測潛在通道是否仍被使用。
...
直接寫 $z\sim q_\phi(z\mid x)$ 會讓一般路徑導數難以穿過抽樣操作。重參數化技巧（reparameterization trick）改寫成
...
第一組應得到非零梯度，第二組的 `requires_grad` 為 `False`、`grad_fn` 為 `None`，連反向都無從開始。這說明重參數化解決的是可微性，不是 KL 與重建之間的張力；把兩者混為一談，會誤以為換個抽樣寫法就能避免後驗坍縮。本段未在本文環境執行。
