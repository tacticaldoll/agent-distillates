...
卷積神經網路（convolutional neural network, CNN）的經典文件辨識系統把局部感受野與共享權重用於手寫字辨識，並在完整文件管線中聯合辨識與其他模組。[LeCun 等人，1998](https://bottou.org/papers/lecun-98h) 本文的核心問題是：這項空間偏置在什麼條件下降低樣本需求，又會在何時刪掉任務需要的自由度？
...
這是平移等變性（translation equivariance）：輸入變換後，特徵圖以對應方式變換。它不是平移不變性；只有後續聚合或讀出忽略位置時，最終決策才可能近似不變。
...
對 stride 1、無 dilation 的 $L$ 層寬度 $K$ 卷積，理論感受野是

$$
R_L=1+L(K-1).
$$

它只表示哪些輸入可能連到輸出。Luo 等人的分析顯示，有效感受野通常只占理論感受野的一部分，影響量還會集中於中央；深度、非線性、下採樣與跳接都會改變分布。[NeurIPS 論文](https://papers.nips.cc/paper/6203-understanding-the-effective-receptive-field-in-deep-convolutional-neural-networks.pdf)
...
`conv` 的誤差應只來自邊界（`roll` 是循環平移，卷積在邊界截斷），`dense` 則沒有理由保持等變。若兩者誤差相近，共享權重並未生效，此時不能把樣本效率歸因於結構偏置。本文環境未安裝 PyTorch，未執行此段。
...
局部性不是普遍真理。若輸出取決於兩個相距很遠的位置，小核心需要深層傳遞才能組合它們；有限深度與集中的有效感受野可能形成瓶頸。全域關係任務是卷積局部偏置的反例，不是單純「資料不足」。
...
另一個錯誤是以理論感受野覆蓋整張影像，便宣稱網路使用了全域資訊。正確對照會測輸入梯度、遮蔽敏感度或干預遠端區域，確認遠端像素是否實際改變決策。

## 結論 (Conclusion)

卷積的樣本效率來自一個可檢查的交換：把位置專屬自由度換成跨位置共享的證據。當資料機制具有局部、可平移重用的規律，這項限制會提高估計效率；當標籤依賴絕對位置、遠距關係或其他變換，它也可能成為錯誤來源。

因此，評估 CNN 應明確回答三個問題：任務具備哪種對稱，輸出需要等變還是不變，以及理論可達範圍是否真的承載影響。結構偏置提供的是可學習捷徑，不是無條件能力。
