...
統計學習（statistical learning）把這個問題寫成三個不同對象：資料生成分佈、可選函數集合，以及選擇函數的準則。深度學習教材也明確區分訓練集上的目標與資料生成分佈上的期望風險；前者只是後者的間接替代量。[Goodfellow、Bengio 與 Courville，第 8 章](https://www.deeplearningbook.org/contents/optimization.html)

本文的核心問題是：觀察到低經驗風險後，還需要哪些條件，才能把它外推成目標分佈上的能力？

## 分析 (Analysis)

### 從低訓練誤差回推三層選擇

設資料集 $S=\{(x_i,y_i)\}_{i=1}^n$ 由分佈 $P$ 抽得，假設空間為 $\mathcal H$。經驗風險（empirical risk）與母體風險分別是
...
經驗風險最小化選擇 $\hat h\in\arg\min_{h\in\mathcal H}\hat R_S(h)$。這個式子定義的是選擇程序，不是泛化保證。從十次曲線的樣本外偏離回推，可得到一條三節點因果鏈：樣本只約束少數位置；較大的 $\mathcal H$ 提供更多同樣貼合樣本的函數；再次使用同一批資料選擇最低誤差者，會把抽樣噪聲也當成可利用訊號。失效點是把「被選中的訓練最小值」當成未經選擇偏差的風險估計。
...
若樣本近似獨立同分佈、損失受控，而且假設空間相對樣本量不過度複雜，才可能用一致收斂或穩定性論證控制 $g$。Shalev-Shwartz 與 Ben-David 的教材把「受限制假設類上的經驗風險最小化」作為正式學習模型的起點，並特別處理單純 ERM 可能過度擬合的條件。[《Understanding Machine Learning》](https://www.cs.huji.ac.il/~shais/UnderstandingMachineLearning/)
...
兩份程式應給出同方向的結論：落差隨 $n$ 縮小、隨容量放大。若只有閉式解出現落差，問題就在擬合程序而不在有限樣本。這段程式不宣稱已執行；本文環境沒有安裝 PyTorch。
...
第一個邊界是分佈偏移。即使 $g(h,S)$ 在 $P$ 下很小，部署分佈若變成 $Q$，真正關心的是 $R_Q(h)$。在 $P$ 上建立的泛化證據不能自動跨越支持集、標註規則或族群比例的改變。

第二個邊界是損失函數。低平均 MSE 不能推出罕見區域、安全成本或校準都良好。損失把多維後果壓成可最佳化數字；沒有被計價的錯誤不會因訓練成功而消失。

反例也很重要。若真實關係確為二次函數、噪聲很低且樣本密集，較大假設空間不必然造成差泛化；正則化也可能讓形式上很大的空間只探索平滑解。因而「參數多」本身不是充分診斷，必須檢查選擇程序實際使用了多少自由度。
...
有限資料不會單獨決定模型能力。它只在假設空間、損失與選擇程序共同建立的通道中排除部分函數。
