...
事實上，數學定理本身並未出錯——卷積運算的平移等變性具備嚴格的群論證明。真正的危機在於：**該定理是一個嚴格依賴於特定邊界條件的「條件命題」；而在工程實作中，標準卷積網路系統性地破壞了定理成立的核心物理前提**。離散網格的下採樣步幅（Strided Sub-sampling）、人為引入的零填充（Zero Padding）邊界，以及深層堆疊下有效感受野的高斯集中衰減，共同促成了幾何性質的崩塌。本文旨在解構連續數學定理與離散工程實作之間的斷層，量化下採樣引發的訊號頻譜混疊（Aliasing）機制，並透過訊號處理的第一性原理建立具備可驗證性的抗混疊濾波工程防線。
