# 買到的是哪幾層：模型輸出與可追責結果之間的交付邊界

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-09T00:12
**Model**: Claude Opus 5
...
把一次「模型產生輸出」到「使用者得到可追責結果」之間的路徑攤開，實務中穩定出現六層。這裡的分析單位是**社會技術系統**（Sociotechnical System）——由技術元件與組織安排共同構成、必須整體運作才產生價值的系統。
...
print("（把 0.80 放在哪一層都一樣，鏈對層的身分是對稱的）")
```
...
（把 0.80 放在哪一層都一樣，鏈對層的身分是對稱的）
```

第一組數字給出一個具體的量級感。每層都達到 $0.97$——在任何單層評估裡這是好成績——端到端只有 $0.833$，每萬件有 1,670 件走不完整條鏈。

第二個結果是這個實驗真正的重點。把那個 $0.80$ 放在模型層、放在資料連接層、或放在救濟層，端到端都是 $0.796008$，一模一樣。**鏈在形式上不區分層的身分。** 因此「模型層最重要」這個判斷若要成立，必須由該場景的實際數值支撐，不能由鏈的結構支撐——而結構恰恰說的是相反的話。
...
主張的第一個邊界是高度標準化的 API 採購。當供應者只承諾服務可用性、客戶自行決定如何使用輸出時，六層裡有四層完全落在客戶側，而合約邊界是清楚的。此時抱怨「供應者沒有交付端到端效用」是誤置的要求。問題會轉移成另一個：客戶側是否知道自己接下了四層。
...
**決定改善哪一層。** 錯誤做法是預設模型層的邊際報酬最高。實測顯示把單一層降到 $0.80$，端到端都是 $0.796008$，鏈對層的身分是對稱的。正確做法是實測各層當前的通過率，把資源投向數值最低的那一層，而那一層經常不是模型。
...
2. **鏈在形式上不區分層的身分。** 把單一層降到 $0.80$，無論放在哪一層，端到端都是 $0.796008$。所以「模型層最重要」需要該場景的數值支撐，不能由鏈的結構支撐。
