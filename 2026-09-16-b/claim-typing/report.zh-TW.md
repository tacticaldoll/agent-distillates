# Artifact 不是 Claim：先替主張定型，才談得上驗證與授權
...
本文建立一套可操作的主張型別（claim typing）。目的不是把自然語言變成繁重本體論，而是阻止一種常見升格：某句話因為進入 proposal 或 spec，就被當成比原本更真的事實。
...
這張表讓「spec 裡寫了」不再是萬用支持。Spec 可以是多種主張的表示位置，但位置不會改變主張所需的證成規則。
...
| 邊界輸入 | Claim type | 關鍵判定／不變式 | 狀態轉移 | 最終處置 |
| :--- | :--- | :--- | :--- | :--- |
...
Proposal 可能同時含 intent、hypothesis 與 design sketch；spec 可能同時含 norm、observable behavior 與 edge-case assumption。一個 completion claim 也可能引用 spec、test result 與 deployment record。將 artifact 與 claim 做一對一綁定，會把文字位置誤當成語意型別。
...
同一 artifact 可以包含多個 claims；同一 claim 也可在 proposal、spec 與 test plan 中有不同投影。穩定身分應屬於 claim，而不是某一段 Markdown 的行號。
...
| human reviewed = true | 身分、角色、範圍缺失 | 把人類存在當合法授權 | 記 approver identity、role、scope、expiry |

矩陣顯示，主張型別不是替文字貼標籤，而是改變後續可接受的推論。
...
Artifact 是協作容器，不是認識論單位。同一份 proposal、spec 或 design 可以同時承載觀察、目的、假說、規範與決策；這些主張需要不同來源、反駁條件與 owner。
