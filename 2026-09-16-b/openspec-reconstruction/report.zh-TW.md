# 把 OpenSpec 重建為變更帳本：Typed Artifacts、證據邊與可撤銷升格
...
重建後的定位是變更帳本（change ledger）：記錄誰提出什麼、哪種行為要改、採取何種設計、什麼 evidence 支持或反駁、誰在何種 scope 核准、上線後發生什麼，以及何時必須撤銷。帳本保存與約束承諾，但不假裝 Markdown 自己創造正當性。

截至 2026-09-16，OpenSpec 官方支援 project-local custom schemas。團隊可 fork `spec-driven`，新增 artifacts、templates 與 dependencies；schema 位於 repository 中並可版本控制（[OpenSpec Customization](https://github.com/Fission-AI/OpenSpec/blob/main/docs/customization.md)）。因此這個方案可以沿用官方擴充點，但簽章、身份、CI refusal 與 production control 仍需外部系統執行。

## 分析 (Analysis)

### 重建目標：讓不同主張不能共用完成旗標

預設 artifacts 主要按工作分工：proposal、specs、design、tasks。重建增加的不是更多散文，而是不同認識論角色：
...
低風險 change 不需要七個獨立文件。Schema 可以用 profile 或條件，把最小路徑壓成三個 typed records：
...
| custom schema 定義 dependencies | 生成 DAG＋外部升格圖 | 官方擴充點 | dependency 不冒充 approval |

這不是在 OpenSpec 外面再造一個互不相干的流程，而是把它定位為可審查的控制面，並讓真正有能力的系統執行身份、證據與 deployment gate。

## 結論 (Conclusion)

OpenSpec 最值得保留的核心不是「spec 驅動一切」，而是 change folder、delta、artifact graph 與可客製 schema。把它重建為變更帳本後，intent、behavior、design、evidence、approval、observation 與 revoke 各有自己的 producer、來源與升格條件。

可靠重建需要兩張圖：schema 管理生成依賴，promotion policy 管理證據與權威。前者讓 agent 高效工作；後者阻止 artifacts 自我證成。Archive 完成 canonicalization 與保存，不終結責任。**好的治理不靠增加散文，而靠型別、來源、權限與可撤銷控制，限制每份文件到底有資格宣稱什麼。**
