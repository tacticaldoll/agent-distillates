...
這種失敗不能簡化成「spec 與 design 沒有分開」。它們確實分開了。真正坍縮的是認識論角色：目的、行為、技術選擇與完成證明都從同一份未驗證的世界模型派生，之後又以彼此一致作為可信度來源。

本文把這種現象稱為規格坍縮（specification collapse）。它不是檔案合併，而是多個應由不同證據或權威支撐的主張，在生成與修正過程中失去獨立性。
...
這五步揭示一個反直覺結果：每一步局部上都可能做對。Spec 忠實承接 proposal，code 忠實承接 spec，test 忠實承接 scenario。全鏈失敗來自第一步的假設從未被暴露，也沒有跨邊界反駁。
...
這就是坍縮的核心不變式：衍生物彼此同意，只證明轉換一致，不能反向證明共同前提正確。
...
這段模型的限制也很清楚：真實 artifact 不會因接觸一個污染來源就全盤錯誤。程式展示的是「未標記假設可沿依賴圖存活」，不是替每份文件判罪。
...
規格坍縮不是文件太少，也不是同一 agent 必然不可信。它發生在目的、行為、設計與完成證明雖分居不同 artifacts，卻從同一個未驗證前提派生，並以衍生物彼此一致反向替前提背書。
