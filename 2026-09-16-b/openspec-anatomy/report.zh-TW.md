...
本文的核心主張是：OpenSpec 的強項是把變更表示成可追蹤的資料結構，並讓 agent 依此協作；它的預設結構不會自行產生需求正當性、外部有效性或組織授權。這不是缺陷指控，而是保證範圍的解剖。
...
| 邊界輸入 | 關鍵判定 | Repository 狀態轉移 | 合理結果 | 仍未回答 |
| :--- | :--- | :--- | :--- | :--- |
...
程式故意不模擬 OpenSpec 實作；它只驗證一個不變式：artifact completeness 與 claim support 是可同時一真一假的獨立狀態。
...
但承載與執行仍需分開。Schema 可以要求先有 `evidence.md` 才產生 `approval.yaml`，卻不會只靠檔名保證證據來自 production、核准者具備權限，或簽章未被偽造。這些控制通常要由 CI、Git hosting、IAM、測試環境與部署平台執行。
