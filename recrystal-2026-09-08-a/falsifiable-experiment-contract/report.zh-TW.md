...
**第一種**：選擇偏差。 同一批資料同時扮演擬合、挑選與證明三個角色。前兩個角色消耗資料中的資訊，第三個角色卻假設資料還是乾淨的。上面的模擬就是這個機制的純粹形態，它的特徵是候選越多、偏差越深。

**第二種**：條件命題的前提破壞。 某個方法或架構宣稱具備某性質，該性質確實可證明，但證明依賴的前提在實作中不成立。它的特徵是理論與實作之間的落差，換更多資料不會改善。

**第三種**：中介誤認。 模型內部有某個可觀測的中間量與輸出相關，於是被當成輸出的原因。它的特徵是觀測本身為真，只有歸因是假的。
