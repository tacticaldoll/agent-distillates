# 移一個像素就翻：卷積不變性宣稱的前提破壞

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T19:31
**Model**: Claude Opus 5
...
這就是**平移等變性**：輸入平移，輸出跟著以同樣方式平移。寫成算子形式，若 $T_\delta$ 表示平移 $\delta$，卷積算子 $C$ 滿足
...
同一類誤判在另一個地方重演。多層堆疊會擴大**感受野**（Receptive Field），也就是一個輸出單元可能連到的輸入範圍。對 stride 為 1、核寬 $K$ 的 $L$ 層網路，理論感受野是
...
隨著深度增加，兩個數字朝相反方向走。理論感受野線性成長到 25，而承載九成影響的寬度只成長到 11——佔比從 100% 掉到 44%。邊緣位置在拓撲上連得到，實際影響卻趨近於零。

這與實測分析的結論一致：有效感受野通常只佔理論感受野的一部分，且影響量集中在中央。[Luo 等人，《Understanding the Effective Receptive Field in Deep CNNs》](https://arxiv.org/abs/1701.04128)
...
前提被違反時，性質不是稍微退化，而是在特定條件下完全失效——量測顯示等變誤差只在平移量為步幅倍數時為零，其餘情況下差異可以大於訊號本身。同樣的結構出現在感受野上：拓撲可達的範圍隨深度線性成長，實際承載影響的範圍成長得慢得多。
