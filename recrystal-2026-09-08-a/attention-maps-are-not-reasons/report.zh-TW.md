# 熱圖不是模型的理由：注意力權重與因果歸因的距離

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T19:58
**Model**: Claude Opus 5
...
先把注意力機制算一次，用三個 token、二維向量，全部手算。
...
現在把最重要的性質指出來。$O$ 是三個值向量的加權和，它是一個**壓縮**。從 $(2.66, 0.98)$ 這個輸出，你無法還原出原本有三個什麼樣的向量。不同的輸入組合可以產生完全相同的輸出——這一點稍後會被量化。
...
這使得注意力權重成為一個**中介變數**：它在因果鏈上，但它既不是起點也不是終點，而且它不是唯一的中介。
...
這意味著在讀熱圖時，你看到的「模型注意到了第 3 個詞」這件事，其中「第 3 個」這個概念是由位置編碼提供的，不是注意力機制本身的能力。若位置編碼在長序列上的外推行為不佳，熱圖上的位置語義也會跟著不可靠。
...
**另一個錯誤做法**：比較長文本模型時同時更換窗口、位置編碼、資料與參數量，再把差異歸因於「注意力機制更好」。
...
這個設計把位置變成唯一的獨立變因。得到的曲線可以回答「路由在多遠的距離開始退化」，而這是一個關於這個模型的事實，不是關於注意力機制的一般宣稱。

## 結論

一個解釋若在被解釋對象消失後依然成立，它解釋的就不是那個對象。這個判準適用於顯著圖，也適用於注意力熱圖。

注意力權重是可觀測的中介變數。它攜帶真實資訊——量測顯示它指向的位置有三分之二的機率確實是影響最大的那個，遠高於隨機。但它也隔著值投影、殘差旁路與後續數十層變換，因此那三分之二的另一面是：每三次就有一次指錯對象。權重衡量混合比例，影響衡量移除後的差異，兩者是不同的量。
