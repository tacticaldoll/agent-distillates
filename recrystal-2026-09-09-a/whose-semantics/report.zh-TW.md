...
沒有任何呼叫者委託過「請解讀我寫進日誌的內容並據以向外取物」。那項擁有權是核心自己長出來的，而它長出來的理由與前一種漂移完全相同：對一個記錄訊息的元件來說，「讓訊息裡可以插值」是一個看起來合理的相鄰職責。
...
> 一項行為屬於核心，若且唯若它的正確值由核心自己的不變式決定。它屬於呼叫者，若它的正確值在不同呼叫者間不同，且各個不同值都不破壞核心的任何不變式。

把它變成一個操作程序：列出兩個以上的真實消費者，逐項問每個消費者該取什麼值，再問取不同值會不會破壞核心的不變式。這裡稱它為變異測試。

### 變異測試

下面的程式把一個持久生命週期核心的六項行為過一遍變異測試。
...
而那四項恰好是背景執行器最常烤進核心的東西。一個真實的重寫可以說明差別。改寫前，核心擁有重試次數、退避、死信政策與排程；改寫後，核心只掌管認領、租約、失效與終局結果，重試與退避的編排完全不做，交給呼叫者以附掛的方式自行決定；持久後端則移到核心之外，各自對同一份一致性測試自證行為。

那次重寫做對的事，正是變異測試的第一欄：它認出「失敗幾次算失敗」在不同業務有不同答案，所以那個值不能由核心挑。
...
這個結果把代價的性質改變了。繞過核心的消費者不只是「用了自己的重試邏輯」——它們同時也離開了核心對認領互斥與終局唯一的裁決，因為繞過的路徑通常繞過整個核心而不是繞過其中一項。**所以誤歸屬的代價不是不便，是核心失去它唯一真正該有的權威。** 一個把應用語義收進來的核心，最後連自己的不變式都保不住。
...
前兩類的動作集合是有限的：一個值，或每個消費者一個值。第三類沒有上界——訊息內容是任意字串，而任意字串可以指向任意端點。

這解釋了修補為何是移除而不是收緊：**一個動作集合無上界的行為，沒有任何「正確的預設值」可以挑。** 變異測試在這一類上不會給出「屬呼叫者」的答案，它會給出「這一項不該存在」。
...
變異測試的操作前提是「兩個以上真實消費者」。這個前提不是形式要求，它是判準能不能執行的條件。

只有一個消費者時，每一項行為的值都「相同」——因為只有一個答案。此時變異測試會把所有行為都判給核心，而那正是把核心焊死在單一消費者形狀上的做法。判準本身沒有錯，是它的輸入不足。

這使歸屬判定依賴於一個更前置的問題：那些真實消費者從哪裡來。那個問題有它自己的困難，本文不處理，但必須指出判準懸在它上面。

## 反思 (Reflection)

判準的第一個邊界是「不變式」這個詞本身。核心的不變式不是自然事實，它是被宣告的。宣告得越寬，越多行為會落進「核心擁有」；宣告得越窄，核心越薄。所以變異測試不能決定核心該有多薄——它只能在不變式已經宣告之後，檢查哪些行為與那份宣告一致。**把不變式寫下來是判準的前置工作，而不是判準的產出。**

一個反例可以標出另一側邊界。假設某項行為的值在各消費者間不同，各值也都不破壞不變式，而把它移出核心之後系統反而更糟——因為每個消費者都實作了一份有 bug 的版本，而核心那一份是對的。這種情形真實存在：正確實作困難、但需求確實因人而異的行為，例如冪等鍵的產生。此時較好的做法不是把它收回核心擁有，而是核心提供一個可替換的預設實作，並讓「可替換」在型別上成立。擁有權與提供實作是兩件事，混為一談會讓這個反例看起來否證了判準。
...
第三個邊界關於第三類。動作集合是否有上界，判定起來不總是清楚。一個接受回呼函式的核心，其動作集合形式上也無上界——但那是呼叫者自己傳進來的函式，動作的授權來源是呼叫者。日誌案例的差別在於動作由**被記錄的資料**觸發，而資料不是授權來源。這個區分是關鍵，而它不在動作集合的大小裡，它在「誰決定了那個動作」。
...
**決定一項行為該不該進核心。** 錯誤做法是問「核心提供這個方便嗎」。實測顯示六項行為裡只有兩項通過變異測試。正確做法是列出兩個以上真實消費者，逐項比對取值，再問取不同值會不會破壞不變式。

**為一項因人而異的行為挑預設值。** 錯誤做法是挑一個「合理的」值並允許覆寫。實測顯示誤擁有一項時，40 個消費者裡有 26 個需要繞過核心，而繞過通常繞過整個核心。正確做法是把該項移出核心，以附掛的方式讓呼叫者自行決定。

**處理一個被濫用的功能。** 錯誤做法是收緊它的允許範圍。日誌案例的最終修法是移除訊息查找替換本身，因為動作集合無上界的行為沒有正確的預設值。正確做法是先判定動作集合有沒有上界；沒有上界就移除，不要調參數。
...
**區分擁有權與提供實作。** 錯誤做法是因為「使用者自己寫會寫錯」而把行為收回核心擁有。正確做法是核心提供一個可替換的預設實作，並讓可替換性在型別上成立——擁有權在呼叫者，實作可以來自核心。

## 結論 (Conclusion)

一項行為屬於核心還是屬於呼叫者，不是口味問題，它有一個可執行的判準：**正確值由核心的不變式決定者屬核心；正確值因呼叫者而異、且各值都不破壞不變式者屬呼叫者。**

本文的量測給出四個結果：

1. **一個持久生命週期核心真正必須裁決的項目很少。** 六項行為過變異測試，只有認領互斥與終局唯一屬核心，其餘四項都是呼叫者的決定。
2. **誤歸屬的代價不是不便，是權威失效。** 只誤擁有一項，40 個消費者裡就有 26 個需要繞過核心；誤擁有三項時只剩 1.7 個仍在核心裁決之下。
3. **存在第三類，它的修法不同。** 動作集合無上界的行為沒有正確的預設值，判準給出的答案是移除而非移出——這正是那次最高評分漏洞最終採取的修法。
4. **判準的輸入是兩個以上真實消費者。** 只有一個消費者時所有值都相同，判準會把一切判給核心，而那就是把核心焊死在單一形狀上。

所以在爭論一個預設值該設多少之前，有兩個問題必須先問：這個行為的動作集合有上界嗎，以及第二個消費者會不會給出不同的答案。
