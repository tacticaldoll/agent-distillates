# 讓越界在編譯期失敗：把守界從記性換成機械

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-09T02:39
**Model**: Claude Opus 5
...
這件事對薄核有直接的含義。一個薄核的邊界若只寫在文件上，它的實際狀態與 `sun.*` 相同：宣告了，沒有生效。而生成端每一次補全都是一次跨越嘗試，頻率比人類開發者高得多。
...
最後一組數字接上另一件事。多擁有的項數會轉成消費者繞過核心的比率，而繞過的消費者同時離開核心對自己不變式的裁決。所以守界失效的終點不是核心變胖，是核心失去它唯一該有的權威。
...
關鍵不是少寫散文，而是**沒有孤兒散文**——每一句治理陳述要嘛會咬，要嘛是焊在某個咬點旁的理由，要嘛明講「這條還不能執行」。三者之外的就是孤兒。
...
同樣是自動檢查，觸發的時機差別很大。一條在 CI 才失敗的規則，與一條在編譯期就失敗的規則，攔截率相同而回饋延遲不同。
...
下圖把咬點按回饋延遲排開。它要回答的閱讀問題是：同一條規則放在不同位置，代價差在哪。
...
第一個邊界是機械檢查自身的成本。一套會咬的治理是要維護的東西，而它也會被慣例引力劃肥——每一條「順便也檢查一下」的提議都有合理的理由。這使守界工具本身需要一個範圍紀律，而那個紀律的內容與它所保護的核心相同：只編碼此刻已經在主張的不變式，其餘記錄延後。
...
第二個邊界關於型別能表達什麼。把藍圖鎖進型別在表達力強的語言裡可行，在動態語言裡大部分做不到。此時能守住的是回饋延遲較長的層——本機檢查與 CI——而攔截率不必然下降，下降的是生成端當場重新推理的能力。這是一個真實的差距，本文不主張它可以被完全補償。
...
最後是實驗的限制。第一個實驗把越界提議數、接受率與攔截率都設成常數且互相獨立，而真實情況下三者相關——越界提議變多時審查者會變嚴。這個負回饋會使文件與人審那兩條路的成長被高估。第二個實驗的八條陳述與它們的分類是我指定的，它示範三分法怎麼算，不評估任何真實治理文件的孤兒率。
...
**維護一套會咬的治理。** 錯誤做法是持續加檢查，因為每一條都合理。正確做法是對治理本身套用同一條範圍紀律：只編碼此刻已在主張的不變式，並在邊界移動時同步移動檢查與理由。
...
4. **同樣的攔截率下，回饋延遲決定生成端能不能當場修正。** 編譯期失敗讓它重新推理一次補全，CI 失敗讓它先寫完一整批。
