# 知識回收的 ROI：把一次性探索轉成可重用理解

**Structure**: Analytical Essay
**Date**: 2026-06-10T17:58
**Model**: GPT-5.5
...
如果探索結束後沒有留下可重用結構，這些成本就會快速消散。下次遇到相似問題時，團隊或個人只能重新展開一次。本文主張：知識回收的 ROI（return on investment）來自把一次性探索轉成可重用理解，而不是來自保存所有對話。
...
真正值得回收的不是每一句話，而是這些被探索成本換出來的結構。蒸餾的工作，就是把暫時性的推理痕跡轉成穩定的理解物件。

### ROI 不是自動為正

知識回收不是保存癖。不是每段對話、每次會議、每個 debug log 都值得結晶。若探索成本低、未來重用機率低，或內容無法形成可校驗結構，蒸餾反而會浪費時間。
...
這個公式提醒我們：蒸餾不是因為「已經花了成本」就必然值得。已花成本只是 sunk cost；真正的 ROI 取決於未來是否能少花同樣的探索成本。

### 哪些探索值得回收

判斷是否值得蒸餾，可以看四個條件：探索成本是否高、重用可能是否高、是否已形成結構、是否可被校驗。
...
這裡的原則是：產物越可操作、越像權威文件，越需要校驗與 owner 授權。蒸餾可以生成候選資產，但不能自動決定它應該升格到哪個層級。
...
這張圖的關鍵是 value gate。沒有重用價值的內容不需要被隆重保存；高重用價值的內容才值得付出蒸餾與校驗成本。
...
好的回收不是保存更多，而是保留能減少未來重複探索的結構。這也意味著，蒸餾的第一個問題不應是「能不能寫成報告」，而應是「這段探索未來是否會再次被需要」。

在 AI 對話中，token 讓成本變得可見；但真正被回收的不只是 token。被回收的是已經展開的問題空間、概念分界、反例與判斷順序。這些東西若能被重用，蒸餾才有 ROI。
...
這個流程把蒸餾從「自動保存」改成「投資判斷」。它保留了知識回收的效率，也避免把所有 context 都固化成資產。
...
當探索成本高、重用可能高、結構已形成且校驗成本可控時，蒸餾通常具有高 ROI。反過來，若只是把低價值內容整理得更漂亮，蒸餾本身就會變成新的浪費。
