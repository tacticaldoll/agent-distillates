...
傳統觀點常將模型失效簡化為「資料量不足」或「模型容量不夠」，並試圖透過單純替換代理指標、放大網路參數量或頻繁重訓來掩蓋問題。然而，在非凸最佳化、高維幾何流形與開放環境動態的交織作用下，能力失效本質上源於五個正交維度的內在物理與數學規律限制：
...
flowchart TD
    ...
    subgraph D2["【表徵維度】流形撕裂與幾何失真"]
        G1["維度壓縮與近似<br/>(量化 / 剪枝 / 蒸餾)"] --> G2["譜半徑截斷與資訊繞道<br/>(後驗坍縮)"]
...
| **結構化資產** | `全部落實` | 每篇報告配備標準四維資產：流程圖 (Mermaid) + 雙重非退化表格 (Walkthrough + Diagnostic) + 跨語言最小驗證程式碼 (Go, Python, Rust, TypeScript, Bash) + 形式化數學/不變式模型。 |
...
- **核心問題**：標量聚合指標為何無法單射到底層機制的健康狀態？在面對子群體漂移與樣本翻轉時，代理指標遞迴為何必然引發 Goodhart 坍縮？
- **不可替代角色**：揭示度量維度的觀測極限，建立多維切片單調性檢驗與辛普森反轉防禦。
- **邊界聲明**：專注於評估管道中的度量失真，不外溢至模型參數量化或訓練最佳化調校。
