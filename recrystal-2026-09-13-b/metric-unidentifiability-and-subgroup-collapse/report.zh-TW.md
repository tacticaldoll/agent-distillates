# 度量自欺與不可識別性邊界：標量讀數下的子群體坍縮與指標遞迴失效
...
若真實系統價值為 $V(f_\theta)$，我們採用代理指標 $M_1$ 作為觀測量。由於 $M_1 \neq V$，最佳化過程會沿著 $\nabla_{\theta} M_1$ 推進，直到進入 $M_1$ 與 $V$ 背離的無效區間；此時團隊引入補丁指標 $M_2$（例如翻轉率懲罰），構造複合目標 $M_{\text{new}} = M_1 - \lambda M_2$。然而，$M_{\text{new}}$ 本身依然是降維產物，其核空間同樣孕育著全新的規避策略（例如以非關鍵特徵的隨機擾動沖銷罰則）。指標遞迴並未消除不可識別性，只是將盲區推移至更高階的導數空間。
...
| **代理指標遞迴** | 引入翻轉率門檻後，翻轉率達標但真實用戶滿意度持續滑落。 | 代理目標與真實價值函數的正交分量被最佳化器過度壓榨。 | 持續疊加輔助懲罰項（加權翻轉、平滑項）。 | **可反駁契約與抽樣對照**：保留未參與最佳化的隨機金標準隔離組，定期對代理指標進行失效證偽。 |
