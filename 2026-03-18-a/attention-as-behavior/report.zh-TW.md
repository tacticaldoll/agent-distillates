# 提及即引導，沉默即邊界 — AI Agent 注意力與多人協作的資訊架構
...
人類的注意力是分層的。當一個人讀到「本專案不涉及模組 A、B、C」時，大腦會將 A、B、C 標記為「已知但不相關」，在後續思考中主動抑制它們。這是選擇性注意力（selective attention）的正常運作——人類擅長忽略被明確標記為不相關的資訊。
...
這不是 agent 的缺陷。這是 transformer 架構的注意力機制的自然特性。context window 中的每個 token 都參與注意力計算，沒有「被標記為不相關因此跳過」的機制。所謂的「忽略」只能靠其他更強的信號壓過，而非靠標記實現。
...
這個差異不是概率性的（「可能會」vs「可能不會」），而是結構性的：指引 A 在 agent 的可操作空間中增加了 4 個實體，指引 B 沒有。可操作空間越大，非預期行為的可能性越高。
...
例如，開發者 A 在 CLAUDE.md 中寫了 Workspace Boundary（正面範圍，不提子模組名稱）。開發者 B 在 config.yaml 的 context 段中寫了完整的子模組列表（出於描述完整性）。agent 同時讀到兩者。CLAUDE.md 說「只碰這些」，config.yaml 說「這些子模組存在」。後者的存在削弱了前者的邊界效果。
