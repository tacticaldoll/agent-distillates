...
這裡發生的首要錯誤不是幻覺，而是**型別誤認**：系統把一個容易繼續生成的候選句，當成已經能約束產品的工程主張。兩者的差異不在文字品質，而在後者必須具備適用範圍、反例、真值來源與失敗後的處置。
...
| 信心分數很高 | 模型內部偏好強 | 把分數映射成業務風險 | 用部署資料校準，分開兩種分數 |
...
最後，機率值並非沒有工程用途。經過適當校準且對應明確事件的機率，可以支援風險排序與資源配置。需要拒絕的是把「某段文字在生成分布中很自然」直接解讀為「該段文字描述的世界很可能為真」。兩者若要建立映射，必須靠外部標註、部署觀察與持續校準，而不能靠語氣完成。
