...
三段結構因此是：先建立不可識別性這個本體（前兩篇），再逐一展示它在具體機制上的樣貌（中間七篇），最後處理方法論自身的限制與可行的程序（後五篇）。
...
| 9 | [模型是常數，風險仍上升：通往下一期資料的三條路](three-paths-into-next-data/report.zh-TW.md) | 參數不變而風險上升時，三條資料路徑如何分流？ | 唯一「模型是常數」的一篇；含標籤延遲下的可觀察程度 | 不涉及模型內部機制 | 分佈漂移、表演性預測、行為切片 |
...
1. **詞庫的跨域撞名**：詞庫以中文詞為主鍵，單一命名空間。`Distill`、`Knowledge`、`Projection`、`Compose`、`DataShape` 五個鍵承載的都是專案治理域的定義，且它們的中文主鍵恰好是機器學習語境常用的字串。錨點注入對中文詞不設字界，因此這些鍵會匹配到 ML 散文的內部，把治理域的定義注入到不相干的段落。這五個鍵在既有語料中的用法都是正確的，所以問題不在定義，而在跨域共用同一個字串。本次以書寫規避處理——寫全稱、改用不撞名的同義詞。作用域機制屬術語治理範圍，移入後續處理。
