...
1. 同一個數字，好幾個世界：能力歸因的不可識別性
2. 存得住不等於學得到：表示能力與最佳化可達性的分離
3. 五十步之外沒有信用：跨時間梯度通道的取得失敗
4. 少了模式，分數不會說：覆蓋損失的證據要另外找
5. 同一個低 KL，四種病因：潛在通道失用的成因分流
6. 新任務如何改寫舊行為：共享方向上的干涉與功能距離
7. 同樣的平均分，不同的失真：三種近似機制的保真度帳
8. 不是後來壞掉，是從頭學錯：偽相關與跨環境反例
9. 模型是常數，風險仍上升：通往下一期資料的三條路
10. 用什麼當基準？判定失效所依據的參考點來源
11. 換一個指標為何不夠：替代量的遞迴不可識別性
12. 契約永遠不完備，所以改守可反駁性
13. 早發現與正確歸因：兩個目標，兩個損失函數
14. 不能做實驗的時候：因果階梯上還剩什麼
...
第 1 篇建立整個系列的問題樣貌。第 10 篇說明所有判決都相對於一個被選的參考點。第 12 篇給出在清單永遠不完備的情況下仍然可守的目標。
