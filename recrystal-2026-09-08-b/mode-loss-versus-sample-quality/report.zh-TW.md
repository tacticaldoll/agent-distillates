# 少了模式，分數不會說：覆蓋損失的證據要另外找

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T21:52
**Model**: Claude Opus 5
...
**分佈覆蓋**（distribution coverage）：目標分佈裡的各個區域有多少被生成分佈觸及。它是對未生成的東西的陳述。
...
操弄變因是生成分佈；控制樣本數（2000）、模式內變異與量測程序；觀察量是品質（每個生成樣本到最近真實樣本的平均距離）與召回（有多少真實樣本被生成分佈覆蓋）。
...
圖的重點是那個菱形閘門。它說明迴圈不是必然自我強化的——它有一個可以量化的抵抗條件。批次內多樣性、目標函數的形式、判別器與生成器的更新比例，這些常被列舉的因素之所以有效，是因為它們都在提高有效的 $\beta$；它們不是各自獨立的偏方，而是同一個量的不同實現途徑。
...
最後，本文的迴圈模型是離散、低維、且把判別器簡化成一個純量懲罰的。它足以證明「坍縮有一個可量化的抵抗條件」這個結構性主張，不足以預測真實訓練中的臨界值。真實系統裡難易差不是常數，會隨生成器改變；$\beta$ 也不是超參數，而是判別器容量、更新次數與正則化共同決定的隱含量。
...
樣本品質是關於已生成樣本的陳述，分佈覆蓋是關於缺席的陳述。任何只檢視已生成內容的協議都無法觸及後者，這是結構上的限制，不是靈敏度不足。
