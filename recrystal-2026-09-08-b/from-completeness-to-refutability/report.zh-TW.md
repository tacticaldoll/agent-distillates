...
所以擴充清單是有價值的動作，而且價值是可量化的——它把殘差裡的一個成分移到可歸因的部分。要注意的是這個判讀成立的條件：我們之所以知道剩下的 $+1.00$ 是交互，是因為在構造這個系統時就知道 $h$ 是最後一個未列座標。真實系統沒有這個保證，所以殘差 $+1.00$ 在那裡仍然同時相容於「交互」與「還有另一個未列座標」。
