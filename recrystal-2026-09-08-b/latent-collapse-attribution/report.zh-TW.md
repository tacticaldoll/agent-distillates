# 同一個低 KL，四種病因：潛在通道失用的成因分流

<!-- front matter -->
**Structure**: Analytical Essay
**Date**: 2026-09-08T22:03
**Model**: Claude Opus 5
...
模型最大化**證據下界**（evidence lower bound）：
...
模型是一個最小的線性高斯系統。編碼器給出 $q(z \mid x) = \mathcal{N}(a x,\, s^2)$；解碼器輸出 $\hat{x} = c z + d b$，其中 $b$ 是一個與 $x$ 相關度為 $\rho$ 的旁路特徵。四個參數以有限差分梯度下降擬合，避免手推導數出錯。
...
量測與情境設定如下。最後兩欄是本文的核心：`構造解損失` 把 $a = 1$、$c = 1$、$d = 0$ 這組明確使用 $z$ 的參數直接放進同一個損失函數求值，不經過訓練。
...
**四種機制在這四欄上無法區分。** 它們共享同一組症狀：KL 貼近零，打亂 $z$ 幾乎不改變輸出。介入測試成功地確認了失用，卻對成因完全沉默。這是本文要解決的問題的具體形狀。
...
圖的關鍵是第二層那個分岔。它不是一個統計檢定，而是一次求值——把一組參數放進損失函數算一個數。成本極低，而它決定了後續四條路徑裡走哪一條。
